...
int main(void)
{
    ...
    OLED_ShowString(2, 1, "Diff Drive Demo");

    /* ── 全自动差速驱动演示 ── */
    /* 1 直线前进 */
    OLED_ShowString(3, 1, "FWD 80  ");
    OLED_ShowString(4, 1, "L:80 R:80  ");
    Car_GoForward(80);
    Delay_ms(3000);
    ...
    /* 3 刹车 */
    OLED_ShowString(3, 1, "BRAKE   ");
    Car_Brake();
    Delay_ms(800);

    /* 4 直线后退 */
    OLED_ShowString(3, 1, "BWD 75  ");
    OLED_ShowString(4, 1, "L:75 R:75  ");
    Car_GoBackward(75);
    Delay_ms(3000);
    ...
    /* 6 左转演示：左75 右100 → 差25%，原地左转 */
    OLED_ShowString(3, 1, "TURN L  ");
    OLED_ShowString(4, 1, "L:75 R:100 ");
    Motor_L_Setspeed(75);
    Motor_R_Setspeed(100);
    Delay_ms(3000);
    ...
    /* 8 右转演示：左100 右75 → 差25%，原地右转 */
    OLED_ShowString(3, 1, "TURN R  ");
    OLED_ShowString(4, 1, "L:100 R:75 ");
    Motor_L_Setspeed(100);
    Motor_R_Setspeed(75);
    Delay_ms(3000);
    ...
    OLED_ShowString(4, 1, "Test Complete");
    Car_Stop();
...
int main(void)
{
    ...
    while (1)
    {
        ...
        int16_t err_display = (int16_t)(err * 10);
        OLED_ShowChar(3, 13, err_display >= 0 ? '+' : '-');
        if (err_display < 0) err_display = -err_display;
        OLED_ShowNum(3, 14, err_display, 2);
...
int main(void)
{
  ...
  OLED_ShowString(1, 1, "ESP AT+RST TEST");
  ...
  /* 1) 发 AT+RST 软重启 */
  OLED_ShowString(2, 1, "AT+RST...     ");
  ESP_SendAT("AT+RST");

  /* 重启后 ESP 返回 "ready"，等久一点 */
  if (ESP_WaitResp("ready", 5000)) {
      OLED_ShowString(2, 1, "RST OK        ");
      ...
      if (ESP_WaitResp("OK", 3000)) {
          ...
          OLED_ShowString(4, 1, "COM OK!       ");
      } else {
          OLED_ShowString(3, 1, "AT -> NO OK   ");
          OLED_ShowString(4, 1, "COM FAIL!     ");
      }
  } else {
      OLED_ShowString(2, 1, "RST NO RESP   ");
      OLED_ShowString(3, 1, "Check wiring  ");
      OLED_ShowString(4, 1, "or baudrate   ");
  }
