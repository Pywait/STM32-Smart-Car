...
static uint8_t state_refresh_needed = 0;   // state transition flag for OLED refresh
...
{
	if (human_detected) {
		ESP_SendAlarm(0x04);  // no person
	} else {
		ESP_SendAlarm(0x03);  // no obstacle
	}

	/* 上发位置信息 */
	uint16_t pos = Position_Get(POSITION_MARKERS);
	if (pos == 0) pos = POSITION_MARKERS;
	ESP_SendPosition(pos);
...
{
	static SystemState prev_state = (SystemState)-1;
	state_refresh_needed = (prev_state != current_state);
	if (state_refresh_needed) prev_state = current_state;
